...
public class FluidHelper {
	...
	public static FluidStack extractFluidFromAdjacentFluidHandler(TileEntity tile, int from, int maxDrain, boolean doDrain) {

		TileEntity handler = BlockHelper.getAdjacentTileEntity(tile, from);

		return handler instanceof IFluidHandler ? ((IFluidHandler) handler).drain(ForgeDirection.VALID_DIRECTIONS[from].getOpposite(), maxDrain, doDrain)
				: null;
	}

	public static int insertFluidIntoAdjacentFluidHandler(TileEntity tile, int from, FluidStack fluid, boolean doFill) {

		TileEntity handler = BlockHelper.getAdjacentTileEntity(tile, from);

		return handler instanceof IFluidHandler ? ((IFluidHandler) handler).fill(ForgeDirection.VALID_DIRECTIONS[from].getOpposite(), fluid, doFill) : 0;
	}
