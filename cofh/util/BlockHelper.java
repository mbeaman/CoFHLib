...
public final class BlockHelper {
    ...
    static {
        rotateType[getId(Blocks.log)] = RotationType.LOG;
        rotateType[getId(Blocks.dispenser)] = RotationType.SIX_WAY;
        rotateType[getId(Blocks.bed)] = RotationType.PREVENT;
        rotateType[getId(Blocks.golden_rail)] = RotationType.RAIL;
        rotateType[getId(Blocks.detector_rail)] = RotationType.RAIL;
        rotateType[getId(Blocks.sticky_piston)] = RotationType.SIX_WAY;
        rotateType[getId(Blocks.piston)] = RotationType.SIX_WAY;
        ...
        rotateType[getId(Blocks.furnace)] = RotationType.FOUR_WAY;
        rotateType[getId(Blocks.lit_furnace)] = RotationType.FOUR_WAY;
        rotateType[getId(Blocks.wall_sign)] = RotationType.SIGN;
        rotateType[getId(Blocks.rail)] = RotationType.RAIL;
        rotateType[getId(Blocks.stone_stairs)] = RotationType.STAIRS;
        rotateType[getId(Blocks.lever)] = RotationType.LEVER;
        rotateType[getId(Blocks.pumpkin)] = RotationType.PUMPKIN;
        rotateType[getId(Blocks.lit_pumpkin)] = RotationType.PUMPKIN;
        rotateType[getId(Blocks.unpowered_repeater)] = RotationType.REDSTONE;
        rotateType[getId(Blocks.powered_repeater)] = RotationType.REDSTONE;
        ...
        rotateType[getId(Blocks.ender_chest)] = RotationType.FOUR_WAY;
        ...
        rotateType[getId(Blocks.hopper)] = RotationType.SIX_WAY;
        rotateType[getId(Blocks.activator_rail)] = RotationType.RAIL;
        rotateType[getId(Blocks.dropper)] = RotationType.SIX_WAY;
    }
        ...
        switch (rotateType[bId]) {
            ...
            case RotationType.CHEST:
                ...
                for (int i = 2; i < 6; i++) {
                    ...
                    if (Blocks.chest.equals(world.getBlock(x, y, z))) {
    ...
    public static int rotateVanillaBlockAlt(World world, int bId, int bMeta, int x, int y, int z) {
        ...
        switch (rotateType[bId]) {
            ...
            case RotationType.CHEST:
                ...
                for (int i = 2; i < 6; i++) {
                    coords = getAdjacentCoordinatesForSide(x, y, z, i);
                    // Yoda conditions, because I can't be bothered dealing with an NPE. :P
                    if (Blocks.chest.equals(world.getBlock(coords[0], coords[1], coords[2]))) {
                        world.setBlockMetadataWithNotify(coords[0], coords[1], coords[2], SIDE_OPPOSITE[bMeta], 1);
                        return SIDE_OPPOSITE[bMeta];
                    }
                }
    ...
    public static boolean isBlock(int bId) {
        // Getting it from the class itself will return Blocks.air if it's null, so this bypasses it.
        return Block.blockRegistry.getObjectById(bId) != null;
    }

    public static boolean isBlock(Block block) {
        int id = Block.getIdFromBlock(block);
        return id > 0 && Block.blockRegistry.containsId(id);
    }
    ...
    public static List<ItemStack> breakBlock(World worldObj, int x, int y, int z, Block block, int fortune, boolean doBreak, boolean silkTouch) {
        ...
        if (silkTouch && block.canSilkHarvest(worldObj, null, x, y, z, meta)) {
            ItemStack itemstack = createStackedBlock(block, meta);
        } else {
    ...
    public static ItemStack createStackedBlock(Block block, int bMeta) {
        ...
        if (isBlock(Block.getIdFromBlock(block)) && Item.getItemFromBlock(block).getHasSubtypes()) {
            return new ItemStack(block, 1, bMeta);
        }
