...
public class EnergyHelper {
	...
	public static int extractEnergyFromAdjacentEnergyHandler(TileEntity tile, int from, int energy, boolean simulate) {
		...
		return handler instanceof IEnergyHandler ? ((IEnergyHandler) handler).extractEnergy(ForgeDirection.VALID_DIRECTIONS[from].getOpposite(), energy,
				simulate) : 0;
	}
	...
	public static int insertEnergyIntoAdjacentEnergyHandler(TileEntity tile, int from, int energy, boolean simulate) {
		...
		return handler instanceof IEnergyHandler ? ((IEnergyHandler) handler).receiveEnergy(ForgeDirection.VALID_DIRECTIONS[from].getOpposite(), energy,
				simulate) : 0;
	}
